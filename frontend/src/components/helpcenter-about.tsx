    ...
    return(
        ...
        <div className="container relative">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 mt-6 gap-6">
                {helpAbout.map((item:HelpAbout,index:number)=>{
                    let Icon = item.icon
                    return(
                        <div className="text-center px-6 mt-6" key={index}>
                            <div className="w-20 h-20 bg-primary/5 text-primary rounded-xl text-3xl flex align-middle justify-center items-center shadow-xs dark:shadow-gray-800 mx-auto">
                                <Icon className="size-7.5"></Icon>
                            </div>

                            <div className="content mt-7">
                                <Link to={item.link} className="h5 text-lg font-medium hover:text-primary">{item.title}</Link>
                                <p className="text-slate-400 mt-3">{item.desc}</p>
                                
                                <div className="mt-5">
                                    <Link to={item.link} className="text-primary">Read More <i className="ri-arrow-right-s-line"></i></Link>
                                </div>
                            </div>
                        </div>
                    )
